...
/**
 * This is a ES6 wrapper class that is constructed by providing a steam api key
...
export class DotaWebAPI {
   ...
   */
  async handleResponse_(response) {
  ...
  async getMatchDetails(matchId) {
    ...
    const queryString = this.buildQueryString_(parameters);

    try {
      const response = await fetch(
        `${API_URL}/${MATCH_INTERFACE}/GetMatchDetails/v1?${queryString}`);
      return this.handleResponse_(response);
    } catch (error) {
      throw error;
    }
  }
   ...
   *
   * @param {string} name
   * @return {string} path to the item icon resource
   */
  getItemIconPath(name) {
    // TODO: Check this string evaluation
    const localizedName = name.replace(/item_/gi, '') + '_';
    return `${STATIC_CDN}/items/${localizedName}lg.png`;
  }

  /**
   * Provide a hero name and image format to collect it's icon image. Default
   * size is 'lg.png'
   *
   * @param {string} name
   * @param {string} size (sb.png, lg.png, full.png, vert.jpg)
   * @return {string} path to the hero icon resource
   */
  getHeroIconPath(name, size) {
    // TODO: Check this string evaluation
    const localizedName = name.replace(/npc_dota_hero_/gi, '') + '_';
    return `${STATIC_CDN}/heroes/${localizedName}${size}`;
  }
}
